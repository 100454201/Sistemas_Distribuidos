...
/*
 * entregar_mensaje: el servidor se conecta al hilo de escucha del cliente
 * destino y le entrega el mensaje segun protocolo seccion 8.6.
 * Devuelve 0 en exito, -1 en error.
 */
 ...
 * segun protocolo seccion 8.6 - ACK.
 */
...
static void op_register(int fd, const char *ip) {
    ...
    if (agregar_usuario(nombre) < 0) {
        pthread_mutex_unlock(&mutex);
        printf("s> REGISTER %s FAIL\n", nombre);
        enviar_byte(fd, 2); /* error memoria */
        return;
    }
...
/*
 * op_connect: conecta un usuario al sistema.
 * Recibe nombre y puerto de escucha del cliente.
 * La IP se obtiene del accept.
...
static void op_connect(int fd, const char *ip_cliente) {
    ...
    while (m != NULL) {
        ...
        if (entregar_mensaje(ip_copia, puerto_copia,
                             m->remitente, m->id, m->texto) < 0) {
            /* Fallo al entregar: marcar usuario como desconectado
               y devolver mensajes no entregados a la lista */
            pthread_mutex_lock(&mutex);
            ...
            /* Liberar el resto de mensajes pendientes */
            m = sig;
            while (m != NULL) {
                sig = m->siguiente;
                free(m);
...
/*
 * op_send: envia un mensaje de un usuario a otro.
 * Protocolo secciones 8.5 y 8.6.
...
static void op_send(int fd) {
    ...
    /* Asignar id y almacenar mensaje */
    unsigned int id = agregar_mensaje(dest, remitente, texto);
    if (id == 0) {
        pthread_mutex_unlock(&mutex);
        enviar_byte(fd, 2);
        return;
    ...
    /* Copiar datos del remitente para la notificacion */
    ...
    /* Si destinatario conectado, intentar entrega inmediata */
    if (dest_conectado) {
        printf("s> SEND MESSAGE %u FROM %s TO %s\n",
               id, remitente, destinatario);

        if (entregar_mensaje(dest_ip, dest_puerto,
                             remitente, id, texto) == 0) {
            /* Entrega exitosa: eliminar mensaje de pendientes */
            pthread_mutex_lock(&mutex);
            ...
            if (rem_conectado) {
                notificar_remitente(rem_ip, rem_puerto, id);
            }
        } else {
...
/*
 * op_users: devuelve la lista de usuarios conectados.
 * Protocolo seccion 8.7.
 * Respuesta: 0 + count + nombres, 1 usuario no conectado, 2 error.
 */
static void op_users(int fd) {
    ...
    /* Copiar nombres de usuarios conectados */
    int  count = 0;
    char nombres[1024][MAX_NAME];
    Usuario *u = lista_usuarios;
    while (u != NULL) {
        if (u->conectado) {
            strncpy(nombres[count], u->nombre, MAX_NAME - 1);
            nombres[count][MAX_NAME - 1] = '\0';
            count++;
        }
        u = u->siguiente;
    }
